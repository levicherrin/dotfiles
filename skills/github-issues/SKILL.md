...
3. **Structure content**: Use the appropriate template from `~/.dotfiles/GITHUB_WORKFLOW.md`
...
Always use the mandatory templates defined in `~/.dotfiles/GITHUB_WORKFLOW.md`. Do not use arbitrary structures or guess the format.
...
Do not guess labels or use standard github default labels. Always strictly follow the `status:` and `priority:` label requirements outlined in `~/.dotfiles/GITHUB_WORKFLOW.md`.
